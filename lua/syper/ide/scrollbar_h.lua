-- Modified DVScrollBar https://github.com/Facepunch/garrysmod/blob/master/garrysmod/lua/vgui/dvscrollbar.lua
-- TODO: make pr to https://github.com/Facepunch/garrysmod
...
--[[
	...
	function PANEL:PerformLayout()
		...
		-- If the scrollbar is enabled make the canvas shorter so it will fit in.
		if ( self.HBar.Enabled ) then Tall = Tall - 16 end

		-- Position the canvas according to the scrollbar's data
		self.pnlCanvas:SetPos( XPos + self.Padding, self.Padding )
		self.pnlCanvas:SetSize( self.pnlCanvas:GetWide(), Tall - self.Padding * 2 )
...
function PANEL:PerformLayout()
	...
	if ( BtnHeight > 0 ) then
		...
		self.btnLeft:SetSize( Tall, BtnHeight )

		self.btnRight:SetPos( self:GetWide() - BtnHeight, 0 )
		self.btnRight:SetSize( Tall, BtnHeight )
